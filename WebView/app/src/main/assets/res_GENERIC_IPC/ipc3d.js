...
function SimulationAPI_set_axes_file(file){
	...
	if(this.is_loaded){
  	if(this._axes_file == ""){
    	...
    	this.control_script.Fields.Item("externalAxes").Add(this.cortona.Engine.CreateVrmlFromString('Inline{url "'+file+'"}'));
    }
	}
}
...
function IPCApi__fitToObject(oPart/*[, fZoomK, sViewname]*/) {
  ...
  var bb = null;
  var n = oPart;
  var isBBoxEmpty = new Function('bb', 'return bb.slice(3).mapand(function _(n) {return n == 0;});')
...
function IPCVrmlNode_isHidden(){
	...
	return (!this.hasGeometry() || this.checkFullTransparency()) && this.getAllChildren().mapand(f);
}
...
Array.prototype.and =
 ...
 Array.prototype.mapor = 
  function Array_mapor(f) 
  {
  	for(var i=0; i<this.length; i++)
  	  if(f(this[i]))
  	    return true;
...
Array.prototype.mapand =
  function Array_mapand(f) 
  {
  	for(var i=0; i<this.length; i++)
  	  if(!f(this[i]))
  	    return false;
