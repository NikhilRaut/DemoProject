 ...
 * hxtable.js 26.12.2013 15:00:41
 *
 */
...
//  Service Functions
//==============================================================================
function navigateLink(uri, target) {
...
function openLink(n, k){
	...
	navigateLink(link.href, link.target);
}
...
function on_simulation_load(bSuccess) {
  if(bSuccess) {
    ...
    if(location.hash)
    {
      ...
      var func = itemnbr.replace(/^([^(]+)\(.*\)$/, '$1').toLowerCase();
      if(func)
        itemnbr = itemnbr.replace(/^[^(]+\((.*)\)$/, '$1');
...
function embed3DControl() {
	...
	if(with3D){
		...
		if(isMSIE){
			...
			cortona = document.getElementById('cortonaControl');
			attachActiveXControlEvents(cortona, ["OnSceneLoaded", "OnSceneUnloaded", "MouseDown", "MouseUp", "OnKeyDown", "MouseMove", "OnMouseOut", "OnMouseEnter"]);
		}else{
	...
	if(with2D){
	  if(isMSIE){
		if(viewer2D==1){
			  ...
			  attachActiveXControlEvents(Control2D, ["HotspotHit", "HotspotEnter", "HotspotLeave", "HotspotMenu", "HotspotMenuSelect"]);
		}else if(viewer2D==2){
				document.getElementById('Container2D').innerHTML = '<object id="isoview2DControl" classid="CLSID:865B2280-2B71-11D1-BC01-006097AC382A" width="100%" height="100%"></object>';
				Control2D = document.getElementById('isoview2DControl');
			  attachActiveXControlEvents(Control2D, ["ObjectHit", "ContextMenuItemHit"]);
		}
	  }else{
...
function isoview2DControl_ObjectHit(nMouseBtn, strObject, sName, KeyState){
	dispose2DContextMenu();
	var hid = strObject;
	var uri = [];
	if(viewer2D==1){
		if(strObject!=""){
			...
			try {
  		  var cgmDoc = Control2D.getWebCGMDocument();
  		  var cgmPic = cgmDoc.firstPicture;
        var gr = cgmPic.getAppStructureById(strObject);
        var uri = gr.getAppStructureAttr("linkuri");
        var firstChar = uri.substring(0,1);
        uri = uri.split(firstChar);
      } catch(e) {}
		}
	}else if(viewer2D==2){
		try{
...
function cortona2DControl_HotspotMenu(HotspotID, MenuObj){
...
function cortona2DControl_HotspotMenuSelect(HotspotID, MenuItemID){
	try{
		eval(cmenuObjs[MenuItemID]);
	}catch(err){}
}

function isoview2DControl_ContextMenuItemHit(item, Title){
	try{
...
function cortona2DControl_HotspotHit(sID, KeyState){return isoview2DControl_ObjectHit(1, sID, "", KeyState);}
function cortona2DControl_HotspotEnter(sID){return isoview2DControl_ObjectHit(0, sID, "");}
function cortona2DControl_HotspotLeave(sID){return isoview2DControl_ObjectHit(0, "", "");}
...
function attachActiveXControlEvents(pControl, pEvents) {
  if('map' in pEvents) {
    void pEvents.map(function(eventname){
      var js = document.createElement('script');
      js.htmlFor = pControl.id;
      js.event = eventname+'()';
      js.id = pControl.id + '_' + eventname;
      js.text = 'return ' + js.id + '.apply(this,  Array.prototype.slice.call(arguments))';
...
function onHashChange() {
  var itemnbr = window.location.hash.substring(1);
  if(!itemnbr) {
    on_simulation_load(true);
    return;
  }
  ...
  if(func)
    itemnbr = itemnbr.replace(/^[^(]+\((.*)\)$/, '$1');
  if(func == 'view') {
    var itemSheet = getDefaultView(itemnbr); 
    preSelectedItem = "";
    if(itemSheet)
	    setTimeout(function() { activateView(itemSheet); }, 100);
	}
	else {
	    setTimeout("workTable.gotoItem('"+itemnbr+"');", 100);
	}
}